...
\gf{GR} represents an emerging framework in \gf{IR} which aims to directly generate document identifiers through autoregressive language models, such as BART \cite{lewis-bart-denoising} and T5 \cite{colin-raffel-exploring-the}, consolidating the retrieval process into a single end-to-end model \cite{peitian-zhang-generative-retrieval,xiaoxi-li-from-matching,tzulin-kuo-a-survey}.

using the \gs{LM} to encode the entire document corpus into the parameters of a single transformer.
...
reduces memory footprint \cite{hansi-zeng-planning-ahead}, enables the model to learn document representations jointly with the retrieval objective \cite{weiwei-sun-learning-to,yubao-tang-listwise-generative} %todo recheck
, and potentially enables the optimization of document representations and retrieval objectives \cite{TODO} %todo too vague? and check
...
In \gs{GR}, a model is trained to map queries to relevant \gp{docid}. In many cases, seq2seq models like BART \cite{lewis-bart-denoising} and T5 \cite{colin-raffel-exploring-the} are used, however recently decoder-only models like LLaMa \cite{alec-radford-improving-language} have also been explored \cite{TODO}.

A fundamental design decision in \gs{GR} is that of \gp{docid}. A \gs{docid} serves as a unique ``index'' for each document or passage, which the model typically must autoregressively generate. Usually, each \gs{docid} links to one document, making it bijective \cite{xiaoxi-li-from-matching} %todo exemptions?
...
% A popular \glsplural{docid} generation method include semantically structured identifiers that are designed to capture the relationships between documents using hierarchical categorization methods, such as by recursively applying k-means clustering to document embeddings, creating a tree where each root-to-leaf path serves as a DocID. Because they share prefixes with similar documents, they facilitate semantic matching, but their sequential nature makes them, susceptible to false pruning. Unstructured atomic identifiers treat each document as a single, unique token added directly to the model's vocabulary. Unlike sequential designs, retrieval involves only a single decoding step, where the model sorts the logits of all \gs{docid} tokens to produce a ranked list. While this makes them immune to false pruning and very FLOP efficient during inference, they require a large increase in model parameters as the corpus scales \cite{ronak-pradeep-how-does,reusch-reverseengineering-the,yi-tay-transformer-memory}}. %This is because atomic \glsplural{docid} predict the document in a single softmax, the output layer must contain one set of weights per document, causing the parameter count to grow with the collection size \cite{thong-nguyen-generative-retrieval,tzulin-kuo-a-survey}.
...
generates a ranked list of docids. Non-autoregressive systems also exist, however, these are the minority \cite{ravisri-valluri-scaling-the,xinpeng-zhao-diffugr-generative}. For standard decoding, encoder-decoder models are the most commonly used, such as BART \cite{lewis-bart-denoising} and T5 \cite{colin-raffel-exploring-the} but decoder-only models such as GPT \cite{alec-radford-improving-language} and LLaMa \cite{aaron-grattafiori-the-llama} for \gs{GR} are explored as well \cite{TODO}.
...
FOr the query, with encoder-decoder models, the encoder embeds the query into a fixed representation, while the decoder uses that to condition \gs{docid} generation. With decoder-only models, the query tokens are encoded implicitly into hidden states for the decoder. %todo recheck

There has also a number of implementations approaches of \gs{GR}, such combining \gs{GR} with \gs{DR} methods \cite{peiwen-yuan-generative-dense}, generating a list of n-grams as identifiers \cite{michele-bevilacqua-autoregressive-search,yongqi-li-multiview-identifiers}, using diffusion models to generate \gp{docid} \cite{xinpeng-zhao-diffugr-generative,qiao-diffusionret-diffusionenhanced}, %how is this special?
using non-autoregressive generation \cite{ravisri-valluri-scaling-the,xinpeng-zhao-diffugr-generative}, and aiming to integrate the entire \gs{GR} process into a single \gs{LLM} \cite{qiaoyu-tang-selfretrieval-endtoend}.
...
One of the problems in GR is the apparent forgetting of fine-grained features \cite{peiwen-yuan-generative-dense}. While generative models capture coarse-grained semantic clusters effectively, they struggle to accurately memorize and decode fine-grained document distinctions \cite{ye-wang-mindref-mimicking}. According to \citet{peiwen-yuan-generative-dense}, error rates (defined as the \% of tokens predicted at a position that does not match the \gs{GT}token in a docid) for NCI \cite{yujing-wang-a-neural} increase significantly at later token positions in \gs{docid} sequences, rising from approximately 1\% at the first position to over 12\% by the sixth position.


\paragraph{Indistinct Identifiers}
\label{par:distinct-ids}

\gs{GR} systems also suffer from cases where the mapping from document content to identifiers is not sufficiently distinct \cite{zhang-multilevel-relevance}. When using synthetic queries \cite{yubao-tang-semanticenhanced-differentiable} % https://arxiv.org/pdf/1904.08375 could be relevant but it just appends the pseudoqueries, not uses as docids
or semantic clustering \cite{yi-tay-transformer-memory} as identifiers, multiple documents may share quasi-identical or highly similar representations, causing the model to conflate distinct documents \cite{cheng-descriptive-and,weiwei-sun-learning-to,xiaoxi-li-from-matching}.

\paragraph{Scalability}
\label{par:scalability}

Scalability limitations become evident as corpus size increases, as \gs{GR} models exhibit limitations in terms of capacity \cite{thong-nguyen-generative-retrieval,hansi-zeng-scalable-and}. The ``parameter budget'' of the transformer can become insufficient to encode nuances of a massive corpus. Empirical studies published by \citet{ronak-pradeep-how-does} show sharp performance decline on T5 \cite{colin-raffel-exploring-the} as corpora scale from 100,000 to 8.8 million passages. For example, the T5-XL model achieves only around 30\% of the MRR@10 score on the full MS MARCO corpus compared to the base corpus.

Associated with scalability is the issue that oftentimes increasing model size does not yield proportional improvements. According to the aforementioned study \cite{ronak-pradeep-how-does}, scaling T5 from XL (3B parameters) to XXL (11B parameters) degraded retrieval performance when using naive numeric identifiers on the full MS MARCO dataset, despite identical training configurations.

Additionally, unstructured atomic identifiers, while considered non-mainstream %https://arxiv.org/pdf/2404.00684
assign each document a single unique integer identifier \cite{yi-tay-transformer-memory}, requiring the model's output vocabulary to equal the corpus size. Since the output projection layer has dimensions of hidden size × vocabulary size \cite{reusch-reverseengineering-the,yi-tay-transformer-memory}, %is this relevant
...
adds approximately 6.8 billion parameters to the output layer alone, making this approach prohibitively expensive for large corpora. In contrast, sequential identifiers reuse a fixed vocabulary across multiple decoding steps, keeping parameter count constant regardless of corpus size \cite{tzulin-kuo-a-survey,ronak-pradeep-how-does}.
...
\paragraph{Dynamic Corpora / Continual Learning}
\label{dynamic-corpora}

Continuously updating the corpora has been a central challenge in \gs{GR}. Several models have been proposed for solving such issues, such as using frozen pre-trained \gp{LM} \cite{varsha-kishore-incdsi-incrementally,guo-corpusbrain-a} %cite better explanation,
and updating only specific \gs{PQ} clusters in \gs{PQ}-based models \cite{chen-continual-learning}


%todo rework this all below in this paragraph
...
We identified the following challenges regarding dynamic corpora/continual learning:
\begin{itemize}
    \item How to effectively expand the system's knowledge (e.g., new discoveries)?
    \item How to efficiently update information (e.g., president changing)?
    \item How to efficiently delete redundant or incorrect information (e.g., fake news)?
    \item How to prevent the model from over-prioritizing newly-added documents (e.g., mistaking them as more important/relevant)?
    \item How to prevent the model from over-prioritizing original/old documents (e.g., mistaking them as more foundational/tested)?
    \item How to enable the model to index and retrieve new documents without needing a full retraining cycle?
    \item How to avoid overwriting old information when not desired, and conversely, how to ensure overwriting when intended?
    \item How to handle new terms that the model’s internal vocab / parametric knowledge does not recognize (e.g. COVID-19, \gs{GR} framework names)?
    \item How to balance the growing corpus size with the need for adding more parameters?
    \item How to avoid the model “hallucinating” the retrieval of a document that has been deleted, especially if a full retraining cycle is not conducted?
    \item How to achieve all these changes in a compute-, time-, and cost-efficient manner?
\end{itemize}
...
Privacy surrounding \gs{LLM}-memorized content and the risk of models producing unsafe content has been a point of great debate nowadays \cite{xiaoxi-li-from-matching} \cite{TODO}

Ensuring generative systems don't violate user privacy or inadvertently return malicious text remains an open challenge.
...
After the \gs{GR} framework retrieved a document (or a list thereof), there usually is a component for generating a response for the user, summarizing the results.

Summarizing with \gp{LLM} provides failure modes such as hallucination \cite{huang-a-survey} and not being up-to-date with latest information \cite{vu-freshllms-refreshing}. These are usually tackled by methods such as self-reflection \cite{jason-wei-chainofthought-prompting,akari-asai-selfrag-learning} or by incorporating a non-parametric memory component, such as \gs{DPR}, to condition generation on retrieved documents at runtime \cite{patrick-lewis-retrievalaugmented-generation}, the latter commonly known as \gs{RAG}.

\subsection{Failure Mode Categorization by Downstream Task}

As explained by \citet{xiaoxi-li-from-matching}, \gs{GR} has mostly been applied and tested \gs{KILT} \cite{petroni-kilt-a} tasks, such as fact verification, open-domain QA, conversational QA, entity linking, multi-hop retrieval, recommender systems, and code retrieval. %todo cite for each?

Accordingly, we can categorize failure modes based on the \gs{NLP} task they are intended for. \textbf{TODO}
%todo I didnt read through these fully yet
% https://arxiv.org/pdf/2404.14851
% kilt stuff: GERE, UGR, DearDR, Re3val ? mentioned by Li
% multihop: https://arxiv.org/abs/2204.13596
% code: https://arxiv.org/abs/2210.00328
% https://dl.acm.org/doi/10.1016/J.IPM.2023.103475
% recommenders explained by Li
%How to make the model perform well across various KILT/NLP tasks?
%How to enable the model to perform well with multimodal data/tasks?
%How to enable models to perform as recommenders?
...
Our methodology is threefold. First, we ran \gs{SEAL} and \gs{MINDER} with their default settings on the \gf{nq} \cite{kwiatkowski-natural-questions}, analyzing their outputs. Second, we will run the models on the \gs{MSMARCO} dataset \cite{payal-bajaj-ms-marco} as well. Following previous papers, we use the MSMARCO 300K and NQ 320K subset.
...
Taking into account the suggestions by \citet{xiaoxi-li-from-matching}, we use the following metrics to determine model quality:
\begin{itemize}
\item R-Precision
\item Recall@k
\item Hits@k
...
For most of our evaluations, we use $k=1$, $k=10$ and $k=100$, following previous works. %cite?
...
discarded the tokens related to the Song Dynasty early on. This example can also be considered a problem in entity linking % really? 
...
is from the Wikipedia article ``Thorn in the flesh,'' which explains that Paul the Apostle used this phrase in 2 Corinthians 12:7--9. \gs{SEAL}'s  top-1 result %TODO report top-1 result or top-k?
...
These examples show that these cases are a mix of such cases. To determine how many fall into each category, in section~\ref{sec:annot}, we apply LLM-as-a-judge to classify whether each retrieved passage genuinely answers the question or not. \textbf{TODO}


\paragraph{Zero-Shot Retrieval}
\label{sec:zero-shot}
If the model is trained through too few epochs, it can underfit and retrieval can be noisy. If it it trained on too many, the model memorizes the training set too well and loses the ability to generalize to new queries. This is especially problematic if the model is expected to perform ``zero-shot'' retrieval. Supervised GR models usually perform significantly worse on datasets they weren't explicitly trained on \cite{yuan-liu-on-the,zhen-zhang-replication-and}. They usually lack robust matching capabilities like that of BM25 or Contriever \cite{izacard-contriever} in zero-shot settings.
...
%Also related, Out-of-distribution retrieval, such as https://arxiv.org/abs/2306.12756}
...
% thic could be a failure of autoreg beam seach
%whether this is failure is debatable
%also this is usually tuneable!
...
\paragraph{Latency}
\label{sec:latency}
Beam search with constraints is probably significantly slower than dot-product (or similar) searches \cite{TODO}
...
If an important identifier (e.g. a proper noun) is split into many meaningless subwords, the model might struggle to generate it consistently compared to common words. This can happen even through BART uses BPE, which allows it to to handle ``out-of-vocabulary'' words by breaking them down into known subunits.

Accordingly, if BART doesn't have good representations for domain-specific jargon, e.g. for rare medical terms, retrieval can fail. 
