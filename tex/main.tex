...
Several %several?
GR solutions use intentional hallucination, such as MINDER \cite{li2023multiviewidentifiersenhancedgenerative}, where the generation of additional identifiers, such as pseudoqueries is achieved through intentional, constrained hallucination of docid generation.

\paragraph{Forgetfulness}
\label{par:forgetfulness}

One of the problems in GR is the apparent forgetting of fine-grained features \cite{yuan2024generativedenseretrievalmemory}. While generative models capture coarse-grained semantic clusters effectively \cite{TODO}, they struggle to accurately memorize and decode fine-grained document distinctions \cite{TODO}. According to \citet{yuan2024generativedenseretrievalmemory}, error rates (defined as the \% of tokens predicted at a position that does not match the GT token in a docid) for NCI \cite{wang2023neuralcorpusindexerdocument} increase significantly at later token positions in \glsxtrshort{docid} sequences, rising from approximately 1\% at the first position to over 12\% by the sixth position.
...
Our methodology is threefold. First, we ran \glsxtrshort{SEAL} and \glsxtrshort{MINDER} with their default settings on the \glsxtrfull{nq} \cite{kwiatkowski-etal-2019-natural}, analyzing their outputs. Second, we will run the models on the MSMARCO dataset \cite{bajaj2018msmarcohumangenerated} as well.%todo explain reasoning why
...
The \glsxtrshort{SEAL} and MINDER output dataset contains the results of 6,515 queries from the \glsxtrlong{nq} benchmark. For each query, the following relevant information is available:

\begin{itemize}
    \item The original natural language query
    \item GT answer string(s)
    \item Annotated GT passages containing the answer
...
From the output data available, the n-gram keys are deemed most useful to understand \glsxtrshort{SEAL}'s and MINDER's retrieval decisions. Each key entry contains three parts:
\begin{enumerate}
