...
\textbf{TODO TIGHTEN THIS PART AS IT IS A BIT BLOATED}
A fundamental design decision in \gs{GR} is that of \gp{docid}. A \gs{docid} serves as a unique ``index'' for each document or passage that the model must autoregressively generate. Usually, each \gs{docid} links to one document, making it bijective \cite{li2025matchinggenerationsurveygenerative} %todo find a specific citation, todo always? todo exemptions?
These can be categorized into several types: unstructured atomic identifiers (such as unique random integers), naively structured string identifiers (such as titles), and semantically structured, numeric identifiers (such hierarchical cluster paths). While atomic IDs require the model to memorize random associations, semantic IDs group similar documents together under shared prefix tokens, improving retrieval quality \cite{pradeep2023doesgenerativeretrievalscale}.

String identifiers use natural language such as document titles, n-grams, URLs, or synthetic pseudo-queries, which allow the model to use its pre-trained linguistic knowledge rather than learning a new symbolic system from scratch. Substring or n-gram identifiers allow any span of text (or group of such spans) within a document to act as its identifier \cite{bevilacqua2022autoregressivesearchenginesgenerating,li2023multiviewidentifiersenhancedgenerative}. Pseudoqueries have also been explored \cite{li2023multiviewidentifiersenhancedgenerative,li2023learningrankgenerativeretrieval,li2024distillationenhancedgenerativeretrieval} as a possible string identifier, shown to prevent forgetting \cite{clever_10.1145/3583780.3614821} and improving query generalization performance \cite{pradeep2023doesgenerativeretrievalscale}. While string-based IDs are more interpretable and generalizable to dynamic corpora where document collections expand, they often suffer from ``false pruning'', where the sequence-based generation discards a relevant document early in the beam search if the model fails to predict the correct prefix token.
