...
In \gs{GR}, a model is trained to map queries to relevant \gp{docid}. In many cases, seq2seq models like BART \cite{lewis-bart-denoising} and T5 \cite{colin-raffel-exploring-the} are used, however recently decoder-only models like LLaMa \cite{alec-radford-improving-language} have also been explored \cite{todo-todo}.

A fundamental design decision in \gs{GR} is that of \gp{docid}. A \gs{docid} serves as a unique ``index'' for each document or passage, which the model typically must autoregressively generate. Usually, each \gs{docid} links to one document, making it bijective \cite{xiaoxi-li-from-matching} %todo find a specific citation, todo always? todo exemptions?
...
%todo can numeric based IDs be refined?

Title-based \gp{docid} have an inherent advantage in corpora with cleanly annotated unique titles such as NQ \cite{kwiatkowski-natural-questions}, but suffer in those without, such as the web-search based MSMARCO \cite{payal-bajaj-ms-marco}. Substring or n-gram identifiers allow any span of text (or group of such spans) within a document to act as its identifier, which allows \textbf{finish sentence}. Pseudoqueries have been shown to prevent forgetting \cite{chen-continual-learning} and improving query generalization performance \cite{ronak-pradeep-how-does}. %todo recheck! also mention the culture thing? or is that in results? also pseudoq also improve distriminativeness or no?
...
%Each type of \gs{docid} design comes with its challenges. While string-based IDs are more interpretable and generalizable to dynamic corpora where document collections expand, they often suffer from ``false pruning'', where the sequence-based generation discards a relevant document early in the beam search if the model fails to predict the correct prefix token. \cite{todo-todo}
...
generates a ranked list of docids. Non-autoregressive systems also exist, however, these are the minority \cite{ravisri-valluri-scaling-the,xinpeng-zhao-diffugr-generative}. For standard decoding, encoder-decoder models are the most commonly used, such as BART \cite{lewis-bart-denoising} and T5 \cite{colin-raffel-exploring-the} but decoder-only models such as GPT \cite{alec-radford-improving-language} and LLaMa \cite{aaron-grattafiori-the-llama}. \textbf{todo recheck to understand well}

Decoding is usually constrained to avoid hallucinating nonexistent \gp{docid}. This is usually done via an FM-index \cite{ferragina-opportunistic-data} (e.g. in SEAL \cite{michele-bevilacqua-autoregressive-search}) or a trie (e.g. in GENRE \cite{nicola-de-cao-autoregressive-entity}). %todo are examples necessary? 

FOr the query, with encoder-decoder models, the encoder embeds the query into a fixed representation, while the decoder uses that to condition \gs{docid} generation. With decoder-only models, the query tokens are encoded implicitly into hidden states for the decoder. \textbf{really? cite? check to confirm understanding.}
...
After the model ranked a list of \gp{docid} an \gs{LLM}...
...
Related to forgetfulness is the question how to enable the generative model to memorize its knowledge sharper. %todo
Similarly, \gs{RAG} for \gs{GR} is also to be explored. %todo these are out of scope
...
adds approximately 6.8 billion parameters to the output layer alone, making this approach prohibitively expensive for large corpora. In contrast, sequential identifiers reuse a fixed vocabulary across multiple decoding steps, keeping parameter count constant regardless of corpus size \cite{tzulin-kuo-a-survey,ronak-pradeep-how-does}. \textbf{recheck this!}
...
In environments where corpora change, \gs{GR} models exhibit two opposing failure modes \cite{chen-continual-learning,zhen-zhang-replication-and,peiwen-yuan-generative-dense,sanket-vaibhav-mehta-dsi-updating}. %are these the right citations?
...
\gs{RAG} has been explored as a way to append new knowledge to the model \cite{todo-todo}
...
Privacy surrounding \gs{LLM}-memorized content and the risk of models product unsafe content has been a point of great debate nowadays \cite{xiaoxi-li-from-matching} \cite{todo-todo} %find some citations
...
After the \gs{GR} framework retrieved a document (or a list thereof), there usually is a component for generating a response for the user, summarizing the results \cite{todo-todo}.
...
As explained by \citet{xiaoxi-li-from-matching}, \gs{GR} has mostly been applied and tested \gs{KILT} \cite{petroni-kilt-a} tasks, such as fact verification, open-domain QA, conversational QA, entity linking, multi-hop retrieval, recommender systems, and code retrieval \cite{todo-todo}.
...
\textbf{duplicate labels}
\textbf{this is an ai generated PLACEHOLDER!}
\clearpage
\includegraphics[width=\linewidth]{assets/taxonomy.pdf}
\clearpage

\section{Methodology}

Our methodology is threefold. First, we ran \gs{SEAL} and \gs{MINDER} with their default settings on the \gf{nq} \cite{kwiatkowski-natural-questions}, analyzing their outputs. Second, we will run the models on the \gs{MSMARCO} dataset \cite{payal-bajaj-ms-marco} as well. We use the MSMARCO 300K and NQ 320K subset. %todo explain reasoning why
...
discarded the tokens related to the Song Dynasty early on. This example can also be considered a problem in entity linking \cite{todo-todo} % really? 
...
\textbf{UNVALIDATED}
\label{sec:zero-shot}
Supervised GR models usually perform significantly worse on datasets they weren't explicitly trained on \cite{yuan-liu-on-the,zhen-zhang-replication-and}.
Since they usually lack robust matching capabilities like that of BM25 or Contriever \cite{todo-todo} in zero-shot settings because they tend to overfit the training query distribution %source? or did I make that up?

\textbf{Also related, Out-of-distribution retrieval, such as https://arxiv.org/abs/2306.12756}
