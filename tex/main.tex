...
In \gs{GR}, a model is trained to map queries to relevant \gp{docid}. In many cases, seq2seq models like BART \cite{lewis-etal-2020-bart} and T5 \cite{t5-raffel2023exploringlimitstransferlearning} are used, however recently decoder-only models like LLaMa \cite{gpt-Radford2018ImprovingLU} have also been explored \cite{TODO}.

A fundamental design decision in \gs{GR} is that of \gp{docid}. A \gs{docid} serves as a unique ``index'' for each document or passage, which the model typically must autoregressively generate. Usually, each \gs{docid} links to one document, making it bijective \cite{li2025matchinggenerationsurveygenerative} %todo find a specific citation, todo always? todo exemptions?
These can be categorized in several ways \cite{li2025matchinggenerationsurveygenerative}. Numeric-based \gp{docid} use (series of) numbers to represent documents. Some implementations include unique random integers \cite{tay2022transformermemorydifferentiablesearch}, treating integers as tokenizable text strings \cite{tay2022transformermemorydifferentiablesearch}, hierarchical cluster paths \cite{sun2023learningtokenizegenerativeretrieval}, RQ\cite{Chen2010-ps-residquant}-based \gs{docid} sequences \cite{ripor_zeng2023scalableeffectivegenerativeinformation}. While atomic IDs require the model to memorize random associations, semantic IDs group similar documents together under shared prefix tokens, improving retrieval quality \cite{pradeep2023doesgenerativeretrievalscale}. Using \gp{LM} to generate \gp{docid} has also been explored \cite{sun2023learningtokenizegenerativeretrieval, ripor_zeng2023scalableeffectivegenerativeinformation}

Text-based identifiers use natural language \gp{docid} which allow the model to use its pre-trained linguistic knowledge rather than learning a new symbolic system from scratch. Among these, the most commonly explored types are document titles \cite{decao2021autoregressiveentityretrieval, corpusbrain-Chen_2022, gere-Chen_2022, zhou2022ultronultimateretrievercorpus}, n-grams \cite{bevilacqua2022autoregressivesearchenginesgenerating,li2023multiviewidentifiersenhancedgenerative,li2023learningrankgenerativeretrieval,li2024distillationenhancedgenerativeretrieval}, URLs \cite{ziems-etal-2023-large, zhou2022ultronultimateretrievercorpus}, synthetic pseudo-queries \cite{li2023multiviewidentifiersenhancedgenerative,li2023learningrankgenerativeretrieval,li2024distillationenhancedgenerativeretrieval, wang2023neuralcorpusindexerdocument, zhuang2023bridginggapindexingretrieval}, and term sets \cite{zhang2024generativeretrievaltermset,zeng2024planningaheadgenerativeretrieval}

%todo can numeric based IDs be refined?

Title-based \gp{docid} have an inherent advantage in corpora with cleanly annotated unique titles such as NQ \cite{kwiatkowski-etal-2019-natural}, but suffer in those without, such as the web-search based MSMARCO \cite{bajaj2018msmarcohumangenerated}. Substring or n-gram identifiers allow any span of text (or group of such spans) within a document to act as its identifier, which allows \textbf{finish sentence}. Pseudoqueries have been shown to prevent forgetting \cite{clever_10.1145/3583780.3614821} and improving query generalization performance \cite{pradeep2023doesgenerativeretrievalscale}. %todo recheck! also mention the culture thing? or is that in results? also pseudoq also improve distriminativeness or no?
Term sets have the benefit of being  permutation invariant \cite{zhang2024generativeretrievaltermset}. Please note that this is not always the case \cite{zeng2024planningaheadgenerativeretrieval}.

\gc{docid} are usually static, but in some implementations they can be refined during training, as evidenced by \citet{lee2023glengenerativeretrievallexical,sun2023learningtokenizegenerativeretrieval}.

%Each type of \gs{docid} design comes with its challenges. While string-based IDs are more interpretable and generalizable to dynamic corpora where document collections expand, they often suffer from ``false pruning'', where the sequence-based generation discards a relevant document early in the beam search if the model fails to predict the correct prefix token. \cite{TODO}

Hybrid docids have also been explored \cite{nguyen2023generativeretrievaldenseretrieval}.



% A popular \glsplural{docid} generation method include semantically structured identifiers that are designed to capture the relationships between documents using hierarchical categorization methods, such as by recursively applying k-means clustering to document embeddings, creating a tree where each root-to-leaf path serves as a DocID. Because they share prefixes with similar documents, they facilitate semantic matching, but their sequential nature makes them, susceptible to false pruning. Unstructured atomic identifiers treat each document as a single, unique token added directly to the model's vocabulary. Unlike sequential designs, retrieval involves only a single decoding step, where the model sorts the logits of all \gs{docid} tokens to produce a ranked list. While this makes them immune to false pruning and very FLOP efficient during inference, they require a large increase in model parameters as the corpus scales \cite{pradeep2023doesgenerativeretrievalscale, 10.1145/3726302.3730076, tay2022transformermemorydifferentiablesearch}}. %This is because atomic \glsplural{docid} predict the document in a single softmax, the output layer must contain one set of weights per document, causing the parameter count to grow with the collection size \cite{nguyen2023generativeretrievaldenseretrieval, kuo2024surveygenerativeinformationretrieval}.
...
generates a ranked list of docids. Non-autoregressive systems also exist, however, these are the minority \cite{valluri2024scalingvocabularynonautoregressivemodels, zhao2026diffugrgenerativedocumentretrieval}. For standard decoding, encoder-decoder models are the most commonly used, such as BART \cite{lewis-etal-2020-bart} and T5 \cite{t5-raffel2023exploringlimitstransferlearning} but decoder-only models such as GPT \cite{gpt-Radford2018ImprovingLU} and LLaMa \cite{grattafiori2024llama3herdmodels}. \textbf{todo recheck to understand well}

Decoding is usually constrained to avoid hallucinating nonexistent \gp{docid}. This is usually done via an FM-index \cite{fm-index-892127} (e.g. in SEAL \cite{bevilacqua2022autoregressivesearchenginesgenerating}) or a trie (e.g. in GENRE \cite{decao2021autoregressiveentityretrieval}). %todo are examples necessary? 
...
There has also a number of implementations approaches of \gs{GR}, such combining \gs{GR} with \gs{DR} methods \cite{yuan2024generativedenseretrievalmemory}, generating a list of n-grams as identifiers \cite{bevilacqua2022autoregressivesearchenginesgenerating,li2023multiviewidentifiersenhancedgenerative}, using diffusion models to generate \gp{docid} \cite{zhao2026diffugrgenerativedocumentretrieval, qiao-etal-2023-diffusionret}, %how is this special?
using non-autoregressive generation \cite{valluri2024scalingvocabularynonautoregressivemodels, zhao2026diffugrgenerativedocumentretrieval}, and aiming to integrate the entire \gs{GR} process into a single \gs{LLM} \cite{tang2024selfretrievalendtoendinformationretrieval}.
...
Continuously updating the corpora has been a central challenge in \gs{GR}. Several models have been proposed for solving such issues, such as using frozen pre-trained \gp{LM} \cite{kishore2024incdsiincrementallyupdatabledocument, 10.1145/3763233} %cite better explanation,
and updating only specific \gs{PQ} clusters in \gs{PQ}-based models \cite{10.1145/3583780.3614821}
...
\gs{RAG} has been explored as a way to append new knowledge to the model \cite{TODO}
...
Privacy surrounding \gs{LLM}-memorized content and the risk of models product unsafe content has been a point of great debate nowadays \cite{li2025matchinggenerationsurveygenerative} \cite{TODO} %find some citations
...
As explained by \citet{li2025matchinggenerationsurveygenerative}, \gs{GR} has mostly been applied and tested \gs{KILT} \cite{petroni-etal-2021-kilt} tasks, such as fact verification, open-domain QA, conversational QA, entity linking, multi-hop retrieval, recommender systems, and code retrieval \cite{TODO}.
...
Our methodology is threefold. First, we ran \gs{SEAL} and \gs{MINDER} with their default settings on the \gf{nq} \cite{kwiatkowski-etal-2019-natural}, analyzing their outputs. Second, we will run the models on the \gs{MSMARCO} dataset \cite{bajaj2018msmarcohumangenerated} as well. We use the MSMARCO 300K and NQ 320K subset. %todo explain reasoning why
...
Taking into account the suggestions by \citet{li2025matchinggenerationsurveygenerative}, we use the following metrics to determine model quality:
\begin{itemize}
\item R-Precision
\item Recall@k
\item Hits@k
...
discarded the tokens related to the Song Dynasty early on. This example can also be considered a problem in entity linking \cite{TODO} % really? 
...
Similar questions can be raised for discriminative language, stereotyping, toxicity, and other forms of culture-related biases \cite{gallegos-etal-2024-bias}
