...
, and potentially enables the optimization of document representations and retrieval objectives %todo too vague? and check
...
\subsection{Failure Modes In Document Retrieval}
...
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}{\end{itemize}}

\begin{tightitemize}
    \item How to effectively expand the system's knowledge?
    \item How to efficiently update information?
    \item How to efficiently delete incorrect information?
    \item How to balance the model's attention to original and new documents?
    \item How to have the model retrieve new documents without a full retraining cycle?
    \item How to avoid overwriting old information when not desired, and, conversely, how to ensure overwriting when intended?
    \item How to handle terms that the model’s parametric knowledge does not recognize?
    \item How to balance the growing corpus size with the need for more parameters?
    ...
    \item How to achieve these in a compute-, time-, and cost-efficient manner?
\end{tightitemize}
...
\label{sec:latency}
\textbf{TODO}
Beam search with constraints is slower than dot-product (or similar) searches.
...
Privacy surrounding \gs{LLM}-memorized content and the risk of models producing unsafe content is a central point \cite{xiaoxi-li-from-matching}. Ensuring generative systems don't violate user privacy or inadvertently return malicious text remains an open challenge.
...
Our methodology is threefold. First, we ran \gs{SEAL} and \gs{MINDER} with their default settings on the \gf{nq} \cite{kwiatkowski-natural-questions}, analyzing their outputs. Second, we will run the models on the \gs{MSMARCO} dataset \cite{payal-bajaj-ms-marco} as well. Following previous papers, we use the MSMARCO 300K and NQ 320K subset. %seal and minder have been ran on Msmarco countless times in other papers
Third, we will perform ablation studies based on our identifier failure modes from the literature review  in Section \ref{sec:failure-modes-literature} and the failure modes identified during our analysis of the outputs. %or maybe no ablation?
