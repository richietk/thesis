...
The additive scoring mechanism in \glsxtrshort{SEAL} introduces a potential bias toward longer documents. Since the final document score $W(d, q)$ is a summation of all matched n-grams, longer documents naturally possess more candidate positions to accumulate scores, potentially outscoring shorter, more precise passages. \glsxtrshort{SEAL} attempts to mitigate this through constraints such as a repetition penalty, a "no-overlap" constraint for n-gram token positions, and exponentially weighting higher-scored n-grams score weighting \cite{bevilacqua2022autoregressivesearchenginesgenerating}. However, these measures primarily target lexical redundancy rather than the underlying advantage provided by larger document length.

This theoretical length bias is not empirically measurable in our experimental setup, as the \glsxtrlong{nq} retrieval corpus consists of length-controlled 100-token passages \cite{karpukhin2020densepassageretrievalopendomain}. Because variation in passage length is minimal, any observed differences in n-gram matching counts reflect \textbf{topical breadth} and the dynamic nature of the n-gram generator rather than document length.
...
\subsection{\glsxtrshort{MINDER} Architecture}
...
We originally hypothesized that cases where the model produces semantically redundant n-grams with high token overlap would constitute a failure mode by inflating scores without providing new evidence. To test this, we measure the unique tokens divided by total tokens across all generated n-grams.

Contrary to our initial hypothesis, the results reveal a negative correlation between diversity and retrieval success ($\rho = -0.227$, $p < 0.001$). Queries with low-diversity (highly repetitive) n-grams achieved a 55.3\% top-1 success rate, compared to only 35.5\% for high-diversity queries.

This finding suggests that repetition is not always a failure of redundancy, but rather an indicator that \glsxtrshort{SEAL} is highly confident in a target document, it generates multiple overlapping n-grams that concentrate score on a specific semantic cluster, which ensures that the additive scoring function accumulates overwhelming evidence for the correct passage.

In contrast, high identifier diversity often indicates a lack of model confidence. In failure cases, the model may generate a wide variety of topically related but lexically distinct tokens that fail to converge on a single passage. Consequently, identifier repetition appears to be a symptom of successful retrieval rather than a cause of failure. This suggests that future scoring functions could potentially use token-overlap not as a penalty, but as a signal of model certainty.
