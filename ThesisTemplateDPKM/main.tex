...
\begin{equation}
    ...
    \label{eq:doc-score}
\end{equation}
where $K^{(d)}$ is the set of non-overlapping n-grams matched in document $d$, $n$ is the specific n-gram being scored in the summation, $\alpha$ is a hyperparameter, and $\text{cover}(n, K(d))$ downweights n-grams whose tokens overlap with higher-scoring n-grams. The purpose of the coverage weight is to avoid the overscoring of repetitive passages, as they contain many overlapping or nearly identical n-grams \cite{bevilacqua2022autoregressivesearchenginesgenerating}. 
...
\textbf{TOBEWRITTEN} %(NQ Dataset details, the 8.46 GT avg, etc)


\paragraph{Available Data Structure}
...
\subsection{Evaluation Metrics} %(Why P@1, why R-Precision, why R-Precision is "fair" for your GT distribution)

\textbf{TOBEWRITTEN}

\subsection{Data Analysis Methods} %(Decile binning strategy, Spearman Correlation explanation)

\textbf{TOBEWRITTEN}

\subsection{Failure Taxonomy}

\paragraph{Observable Failure Modes}
...
\paragraph{Non-Observable Failure Modes}
...
\paragraph{Failure Mapping} %(Mapping the abstract failures from Section 2.5 to the observable metrics like unigram proportion) EITHER THINK THROUGH THAT OR REWORK THE WHOLE SUBSECTION OR DEBLOAT OR REWRITE OR DELETE OR IDK.
\textbf{TOBEWRITTEN}
\textbf{UPDATE AND REWORK THIS TO FIT THIS BUT THIS SHOULD BE IN THE METHODOLOGY}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrr}
        ...
        \midrule
        $\mu$ Score (Unique $P$)             & 5.67   & 6.12   \\
        $\mu$ Score (Unique $N$)             & 6.66   & 6.94   \\
        $\mu$ Score (Shared)                 & 17.28  & 17.34  \\
        \bottomrule
    \end{tabular}
\end{table}
...
According to the methodology explained in Section \ref{sec:llm-judge}, we adjust the estimate of \glsxtrshort{SEAL}'s and \glsxtrshort{MINDER}'s retrieval performance. Table~\ref{tab:adjusted-recall} presents the adjusted hits@2 under different inclusion criteria. \textbf{MINDER DATA IS TBD. TODO}

\begin{table}[htbp]
    \centering
    \caption{Adjusted hits@2 estimates based on LLM-as-a-judge results.}
    \label{tab:adjusted-recall}
    \begin{tabular}{l|rr|rr}
        \toprule
        & \multicolumn{2}{c|}{\textbf{\glsxtrshort{SEAL}}} & \multicolumn{2}{c}{\textbf{\glsxtrshort{MINDER}}} \\
        \textbf{Inclusion Criterion} & \textbf{Hits@2} & \textbf{\%\textsuperscript{*}} & \textbf{Hits@2} & \textbf{\%\textsuperscript{*}} \\
        \midrule
        GT passage only (baseline) & 3,416 & 52.4\% & 3759 & 57.7\% \\
        + YES verdicts             & 3,624 & 55.6\% & TBD & TBD \\
        + YES and PARTIAL verdicts & 3,726 & 57.2\% & TBD & TBD \\
        \bottomrule
        \multicolumn{5}{l}{\small \textsuperscript{*}Percentage of the total 6,515 queries.}
    \end{tabular}
\end{table}

When counting only YES verdicts, \glsxtrshort{SEAL}'s effective hits@2 increases from 52.4\% to 55.6\%, a 3.2 percentage point improvement. Including borderline cases (YES + PARTIAL) raises the estimate to 57.2\%. These adjustments suggest that \glsxtrshort{SEAL}'s true retrieval capability may have been underestimated by approximately 3--5 percentage points due to incomplete or inaccurate ground-truth annotations. \textbf{MINDER DATA IS TBD.} Please note that we will refrain from ``updating the dataset'', as that would make our results incomparable with other papers' results on the standard \glsxtrshort{nq} dataset.
...
\paragraph{Preference for Unigrams \& Effects}
\label{sec:unigrams}
...
We quantify this generation bias by measuring the fraction of generated n-grams that consist of a single token in the top-1 retrieved passage. Across 6,515 queries, the mean unigram proportion is 0.869 (median 0.878) for \glsxtrshort{SEAL}, 0.853 (0.863) for \glsxtrshort{MINDER}, respectively, and the average n-gram length is 1.28 words (median 1.24) for \glsxtrshort{SEAL}, and 1.48 (1.35) for \glsxtrshort{MINDER}, respectively. indicating that, on average, more than four-fifth of generated n-grams are unigrams for both systems. This indicates a strong skew toward short n-grams despite \glsxtrshort{SEAL}’s ability to generate longer sequences and \glsxtrshort{MINDER} specifically generated long pseudoqueries occasionally being used as n-grams.

We categorize queries into deciles based on the unigram proportion. Queries in D1 have lower unigram proportion than in D10 and achieve considerably higher hitrate both with $k=1,\space k=10$, as evidenced by Table \ref{tab:unigram-dec}. This indicates that passages with a higher proportion of multi-word n-grams are substantially more likely to retrieve the correct document, while passages dominated by unigrams correspond to poorer retrieval performance.
...
Table \ref{tab:answer-presence-impact} illustrates the results of out analysis. We looked at whether the top-scored retrieved passage contained the answer string or not and what the Hits@1 value is for these cases. The results are mostly similar between \glsxtrshort{SEAL} and \glsxtrshort{MINDER}, with most top-1 ranked retrieved passages not containing the answer string, yet when they do, the respective query's Hits@1 rate doubles.
\textbf{TODO FINISH THIS. maybe try to do Of passages that have the answer, what \% are ground truth? and Of ground truth passages, what \% have the answer?}
\begin{table}[htbp]
    \centering
    \caption{Performance metrics by answer string presence.}
    \label{tab:answer-presence-impact}
    \begin{tabular}{l|rr|rr}
        \toprule
        & \multicolumn{2}{c|}{\textbf{\glsxtrshort{SEAL}}} & \multicolumn{2}{c}{\textbf{\glsxtrshort{MINDER}}} \\
        \textbf{Metric} & \textbf{Ans. Pres.} & \textbf{Ans. Abs.} & \textbf{Ans. Pres.} & \textbf{Ans. Abs.} \\
        \midrule
        \textit{Query Level (top-1 psg)} & & & & \\
...
\paragraph{Misranking Despite Retrieval}
\label{sec:failure-2}
...
\paragraph{Overreliance on a Single Identifier} %maybe change to top-5? or idk, gotta think through this
\label{sec:failure-6}
...
\paragraph{Impact of N-gram Quantity on Retrieval Success}
