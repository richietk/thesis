...
occurrence. The parameter $\alpha$ is a hyperparameter. The coverage weight 
$\text{cover}(n, K^{(d)})$ downweights individual n-grams whose tokens 
overlap with tokens in higher-scoring n-grams, preventing passages from 
receiving inflated scores when they contain many lexically similar n-grams.\cite{bevilacqua2022autoregressivesearchenginesgenerating}. %todo what is w?
