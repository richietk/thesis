...
\subsection{\glsxtrshort{SEAL} Architecture}
...
Architectural constraints pose a significant challenge in many \glsxtrshort{gr} approaches. As an example, \glsxtrshort{docid} assignment is traditionally non-differentiable because model architectures typically use the argmax function to select discrete \glspl{docid} from a probability distribution during training, breaking gradient flow and preventing joint end-to-end optimization \cite{yang2023autosearchindexerendtoend, sun2023learningtokenizegenerativeretrieval}. While \glsxtrshort{gr} aims toward single unified models, the highest-performing systems often introduce additional components, specific constraints, or multi-stage training processes, which break end-to-end differentiability in most cases. \cite{tang2024generativeretrievalmeetsmultigraded, 10.1145/3603167, tang2024listwisegenerativeretrievalmodels}.

\subsubsection{Document Length Bias}

The n-gram aggregation mechanism in \glsxtrshort{SEAL} introduces a theoretical bias toward longer documents in corpora with variable document lengths. Because \glsxtrshort{SEAL}'s default configuration uses pure score aggregation, where each document's final ranking is the sum of scores from all matched n-grams, longer documents have more textual positions available to match different n-grams, potentially accumulating higher scores regardless of their relevance and specificity. A short, highly-relevant passage containing a few highly-specific n-grams could be outscored by a longer, topically-related but less relevant document containing many generic n-grams that individually contribute modest scores but collectively dominate.

The \glsxtrshort{SEAL} paper does not explicitly address this as a potential failure mode, focusing instead on the model's ability to generate diverse n-grams and overcome vocabulary mismatch. However, the implementation includes several mechanisms designed to mitigate score accumulation from redundant or overlapping evidence. First, a repetition penalty $\beta$ reduces scores for n-grams containing tokens already covered by previously-scored n-grams in that document. If an n-gram's tokens have all appeared in earlier matches, its score is multiplied by $(1-\beta)$ where $\beta \in (0,1)$. Second, a no-overlap constraint prevents multiple n-grams from scoring the same token positions within a document so that each piece of text contributes at most once. Third, exponential score weighting $\text{score}^{\alpha}$ amplifies the difference between high-scoring and low-scoring n-grams, theoretically favoring documents with fewer but more relevant matches. Finally, the \texttt{single\_key} parameter allows interpolating between pure score aggregation and ranking by best single n-gram, though it defaults to pure aggregation.

These countermeasures provide partial mitigation but remain imperfect under default settings. The repetition penalty only reduces redundancy among overlapping tokens, not semantically redundant but lexically distinct n-grams (e.g., ``earthquake data'' and ``seismic data'' would both contribute nearly full scores despite conceptual overlap). The non-overlapping constraint prevents position reuse but does not address the fundamental issue that longer documents have more positions available to match different n-grams. The exponential weighting helps but is not a robust solution when the model generates many medium-relevant n-grams. The most effective mitigation, increasing \texttt{single\_key} to emphasize best-match quality over quantity, is disabled by default and not discussed in the original paper as a tuning consideration.

\paragraph{Inapplicability to the \glsxtrlong{nq} Corpus}
Importantly, this theoretical document length bias cannot be empirically tested in our experimental setup. The \glsxtrlong{nq} retrieval corpus used by \glsxtrshort{SEAL} consists of a Wikipedia dump split into approximately 21 million passages of 100 tokens each \cite{karpukhin2020densepassageretrievalopendomain}. Because all passages are length-controlled by construction, variation in passage length is minimal and any differences in n-gram matching counts between passages cannot be attributed to document length.

Instead, in our length-controlled corpus, the number of n-grams matched to a passage reflects topical breadth rather than document length. A passage that matches many generated n-grams does so because its content is topically broad, containing vocabulary from multiple aspects of the query's semantic domain, not because it is longer. Conversely, a passage matching few n-grams may be highly specific or may simply lack lexical overlap with the generated n-grams. This distinction is important for interpreting the empirical results in Section~\ref{sec:failure-pattern-analysis}, as when we observe that incorrect (``negative'') passages match more n-grams than correct (``positive'') passages, this indicates that \glsxtrshort{SEAL} generates n-grams favoring topically broad but potentially less relevant content, rather than evidence of length-based score inflation.

\section{Methodology}

\subsection{Failure Modes in \glsxtrlong{gr}} 
...
One of the most common failure modes is when generated n-grams lack sufficient specificity to distinguish relevant documents from semantically related but incorrect alternatives.

The n-gram \texttt{`` Reba''} in Table~\ref{tab:ngram-example} appears 1,851 times across the corpus, meaning this query-relevant term matches thousands of documents about Reba McEntire unrelated to the specific song. Similarly, \texttt{`` country singles''} (frequency: 777) and \texttt{`` country music singer''} (frequency: 4,024) are topically relevant but not specific enough to isolate the correct passage. We can see this issue especially if the top-scoring n-grams have a high corpus frequency, if n-grams are matching many passages beyond the ground truth, and if the query relevant terms are common across the whole topical domain. Cases where the model generates topically appropriate but overly generic n-grams are thus a key issue. The corpus frequency directly quantifies specificity, as an n-gram which appears countless times over and over provides minimal discriminative signal regardless of its relevance to the query.

While Equation~\ref{eq:ngram-score} downweights frequent n-grams via the $P(n)$ term, if the model assigns high conditional probability $P(n|q)$ to generic n-grams, they may still dominate the final ranking. This occurs because \glsxtrshort{SEAL} is trained to generate n-grams from ground-truth passages that have high lexical overlap with queries, meaning it learns to reproduce query-relevant vocabulary. However, this training signal does not distinguish between highly specific terms (like song titles or unique names) and broadly applicable domain terms (like ``country music singer''). When a query clearly indicates a topical domain, such as country music, sports, or politics, the language model's pre-trained knowledge causes it to assign high conditional probability to domain-typical phrases, regardless of whether those phrases discriminate the specific answer passage from thousands of other passages in the same domain. For instance, a query about a specific Supreme Court case might generate ``Supreme Court'' with high $P(n|q)$, which, despite having corpus frequency over 10,000, still receives a non-trivial score because the model's confidence in its relevance partially overcomes the frequency penalty.
...
\textbf{Example 1: ``which apostle had a thorn in his side'' (answer: Paul).} The ground-truth passage is from the Wikipedia article ``Thorn in the flesh,'' which explains that Paul the Apostle used this phrase in 2 Corinthians 12:7--9. \glsxtrshort{SEAL}'s top result is a 100-word chunk from Galatians 3, which mentions ``Paul the Apostle'' as the author of that epistle but contains nothing about a thorn. Even though \glsxtrshort{SEAL} seemed to be able to identify ``Paul'' as a relevant term (potentially from BART's pretrained parameters), this is still a failure, since Galatians 3 is the wrong chapter entirely. \glsxtrshort{SEAL} matched on generic terms (``Paul,'' ``apostle'') but never generated n-grams containing ``thorn.'' No other chunk from Galatians would help, as the thorn passage is in 2 Corinthians, a different book of the Bible.

\textbf{Example 2: ``how many judges currently serve on the supreme court'' (answer: nine).} The ground-truth passage directly states the current composition of the Court. \glsxtrshort{SEAL}'s top result is a historical passage stating that in 1869, ``the Circuit Judges Act returned the number of justices to nine, where it has since remained.'' This is borderline, because the reader could infer the answer, but the passage is about the 19th-century history of the Supreme Court, not specifically about the current number of judges.

\textbf{Example 3: ``who is the youngest judge currently sitting on the u.s. supreme court'' (answer: Neil Gorsuch).} The ground-truth passage discusses Gorsuch's nomination process and him being the youngest sitting justice. \glsxtrshort{SEAL}'s top result directly states: ``Neil Gorsuch is the youngest justice sitting, at 50 years of age.'' This is a success, because \glsxtrshort{SEAL} found a passage that answers the question even more directly than the annotated ground truth.

These examples show that the 426 cases are a mix of genuine successes where \glsxtrshort{SEAL} found an answer in a different but valid passage (Example 3), some borderline cases requiring inference (Example 2), and some failures where the answer string appears coincidentally without answering the question (Example 1). To determine how many fall into each category, manual review or LLM-based classification is required. Section~\ref{sec:llm-judge} applies LLM-as-a-judge to classify whether each retrieved passage genuinely answers the question or not. If most of the 426 cases are genuine retrievals, then \glsxtrshort{SEAL}'s effective top-2 recall is closer to 59.0\% rather than 52.4\%. If most are coincidental matches, then 52.4\% remains the accurate figure.
...
For this, it would be useful to understand why \glsxtrshort{SEAL} is capable of such rephrasing. During the generation phase, \glsxtrshort{SEAL} uses the FM-index to enforce that at each decoding step, the language model can only generate tokens that would extend the current partial n-gram into a sequence that exists somewhere in the corpus. Specifically, when \glsxtrshort{SEAL} has generated the partial sequence ``earthquakes can be,'' the FM-index queries which tokens follow this exact 3-gram in the corpus and constrains generation only to those possibilities. This means that ``earthquakes can be predicted'' can only be generated if this complete 4-gram exists as a contiguous substring in at least one document. 

After n-gram generation, in the retrieval phase, for each n-gram, \glsxtrshort{SEAL} uses the FM-index to identify every document containing that n-gram and assign a relevance score based on the n-gram's uniqueness and frequency in the corpus. Documents are then ranked by aggregating the scores from all n-grams they contain. This means that a document scores highly not because any single n-gram uniquely identifies it, but because it contains multiple high-scoring n-grams. For example, even if ``earthquakes can be predicted'' appears in only one document, that document's final ranking depends on whether it also contains other highly relevant generated n-grams like, for example ``seismic activity'' or ``forecasting methods.'' This provides robustness, since even if one highly ranked n-gram points to an irrelevant document, other n-grams can still guide retrieval toward more relevant documents that contain a higher concentration of query-related terms.

This architecture helps explain why \glsxtrshort{SEAL} can overcome vocabulary mismatch issues. While the model cannot generate n-grams that do not exist in the corpus, it can find paraphrases and related terms as long as those appear in corpus documents.

This limitation affects the interpretation of failure cases in the empirical analysis, because when \glsxtrshort{SEAL} fails to retrieve a relevant passage, the failure may stem not from the absence of relevant terms in the document, but from the model's inability to generate a valid n-gram sequence that connects the query phrasing to the specific word ordering in the passage. Therefore we must examine whether alternative ngrams exist that could've plausibly identified the document. Additionally, using skip-grams instead of ngrams could theoretically be an alternate solution to the problem. That is, however, out of scope for this thesis.
...
\begin{table}[H]
    \centering
    \caption{Summary of failure mode analysis for \glsxtrshort{SEAL} on \glsxtrlong{nq} (N=6,515).}
    \label{tab:failure-summary}
    \begin{tabular}{p{0.22\textwidth} r p{0.38\textwidth} r}
        \toprule
        \textbf{Failure Mode} & \textbf{Spearman $\rho$} & \textbf{Success Rate Comparison} & \textbf{$\Delta$} \\
        \midrule
        Bias Toward Short Identifiers & $-0.278$ & Low unigram frac.: 80.8\% vs.\ high: 27.5\% & +53.3 pp \\
        \addlinespace
        Insufficient Query Coverage & $+0.290$ & High overlap: 60.2\% vs.\ low: 20.2\% & +40.0 pp \\
        \addlinespace
        Answer in n-grams & --- & Present: 75.0\% vs.\ absent: 34.9\% & +40.1 pp \\
        \addlinespace
        High-freq.\ n-grams & $-0.238$ & Low freq.: 59.8\% vs.\ high freq.: 32.5\% & +27.3 pp \\
        \addlinespace
        Correlated identifier generation\textsuperscript{\dag} & $-0.227$ & Low diversity: 55.3\% vs.\ high: 35.5\% & +19.8 pp \\
        \addlinespace
        Overreliance on a Single Identifier & $-0.115$ & Distributed: 48.5\% vs.\ concentrated: 29.4\% & +19.1 pp \\
        \addlinespace
        Misranking Despite Retrieval & --- & 55.0\% of retrieved passages mis-ranked & --- \\
        \bottomrule
    \end{tabular}

    \vspace{0.5em}
    \footnotesize
    \textsuperscript{\dag} Correlated identifier generation shows an inverse relationship: lower diversity (more repetition) correlates with \textit{higher} success, suggesting focused n-gram generation benefits retrieval rather than harming it. \\
    pp = percentage points. $\Delta$ = difference in top-1 success rate between favorable and unfavorable conditions.
\end{table}
...
Decile-based binning of the unigram fraction similarly shows the success rate decline as the unigram fraction increases, from 67.7\% in the lowest decile (0.51–0.78) to 22.6\% in the highest decile (0.94–1.0). The Spearman correlation $\rho$ between unigram fraction and top-1 retrieval success is $-0.278 \space (p<0.001)$, indicating a statistically significant, but moderate negative relationship between unigram dominance and retrieval effectiveness.
...
\begin{figure}[H]
    ...
    \caption{Top-1 retrieval success rate by mean fraction of unigrams among n-grams generated (decile bins)}
\end{figure}

\subsubsection{Correlated Identifier Generation}
\label{sec:failure-5}

Correlated identifier generation occurs when \glsxtrshort{SEAL} produces semantically redundant n-grams containing repeated tokens across multiple generated sequences. For example, a query about country music might generate ``country,'' ``country music,'' ``country singer,'' and ``country charts,'' all sharing the token ``country.'' While these n-grams are individually valid, their redundancy can inflate scoring for passages containing common topical terms without ensuring those passages contain the specific answer.

We measure repetitiveness using the number of unique tokens divided by the total tokens across all generated n-grams for a query. A ratio near 1.0 indicates each token appears only once, while lower ratios indicate repeated tokens across multiple n-grams. We arbitrarily define low diversity as $<$0.7, medium as 0.7-0.85, and high as $>$0.85. Across 6,515 queries, the mean diversity ratio for the top-1 retrieved passage is 0.881, with 114 queries (1.7\%) exhibiting low diversity, 1,941 (29.8\%) showing moderate diversity, and 4,460 (68.5\%) achieving high diversity.

Counter-intuitively, the results reveal a weak, but significant \textit{negative} correlation between diversity and success ($\rho = -0.227$, $p < 0.001$). Low-diversity queries achieve a 55.3\% success rate and medium-diversity 55.1\% compared to 35.5\% for high-diversity queries. This +19.8 percentage point advantage for low-diversity generation contradicts the initial hypothesis that correlated identifier generation constitutes a failure mode.

This unexpected finding suggests that such repetition may actually benefit retrieval. When \glsxtrshort{SEAL} repeatedly uses key topical terms across multiple n-grams (e.g., generating ``Reba,'' ``Reba McEntire,'' ``singer Reba'' for a question about Reba McEntire), it reinforces topical focus and ensures that the scoring function accumulates substantial evidence for passages containing those terms. In contrast, high diversity may indicate that the model is exploring too broad a semantic space, generating n-grams that individually score well but collectively lack the coherent focus on a single target passage. Consequently, rather than classifying correlated identifier generation as a failure mode, it appears to function as a successful retrieval strategy wherein the model concentrates its generated evidence on a specific semantic cluster.
...
Coverage varies substantially across the top-1 retrieved passages. For many queries, the lexical overlap between the query tokens and the top-1 passage is low-to-moderate, with only a minority of queries achieving high overlap.

The relationship between query coverage and retrieval success, while not strong, is statistically significant. Queries whose top-1 retrieved passage has high lexical overlap (defined as $>$60\%) achieve a 60.2\% success rate, compared to 41.9\% for mid-coverage (defined as 30\%-60\%) and 20.2\% for low-coverage (defined as $<$30\%) queries ($\rho = 0.290$, $p < 0.001$). This demonstrates that higher lexical overlap in the top-1 passage correlates with a higher likelihood of retrieving a correct passage.

Low query coverage typically arises in two scenarios. First, the model may generate semantically related but lexically distinct n-grams (e.g., for a query containing ``automobile,'' the model could generate n-grams with ``car'' or ``auto''). Second, the model may generate generic topical n-grams that lack the specific query terms needed to isolate the answer (e.g., a query about ``Victorian architecture in London'' generates only ``architecture'' and ``London''). In both cases, while the generated n-grams may retrieve topically relevant passages, they fail to discriminate between documents enough to retrieve the correct one.
...
\subsubsection{Overreliance on a Single Identifier}
\label{sec:failure-6}

Overreliance on a Single Identifier measures the extent to which the top-1 ranked passage's total score is concentrated in one high-scoring n-gram versus distributed across multiple n-grams. We define this ``dominance'' as the ratio of the highest scored n-gram score and the sum of all matched n-gram scores for the passage. A high ratio indicates that a single n-gram contributes more than half the total score, while a low ratio suggests distributed evidence from multiple n-grams (or, alternatively, low model confidence) .

Across 6,515 queries, the mean such ratio is 0.438 (median 0.439), indicating that on average, the top-scoring n-gram contributes approximately 44\% of the total score. The distribution reveals that 698 queries (10.7\%) exhibit a ratio of $<$0.35, while 3972 queries (61.8\%) exhibit a ratio between 0.3 and 0.5, and 1845 queries (28.3\%) have more than half of their total score contributed by a single n-gram.

The relationship between such single-query dominance and retrieval success demonstrates a weak but clear negative correlation (Spearman $\rho = -0.115$, $p < 0.001$). Stratified into 10 bins, queries in the lowest ratio bin (0.12–0.30) achieve a top-1 success rate of approximately 48.5\%, while queries in the highest bin (0.57–0.85) achieve only 29.4\%.

This finding aligns with \glsxtrshort{SEAL}'s intersective scoring design, which aggregates evidence from multiple n-grams to identify passages. Theoretically, when retrieval succeeds through distributed evidence, multiple n-grams collectively point toward the target passage, providing redundant confirmation and reducing sensitivity to individual n-gram errors. Conversely, when a single n-gram dominates, retrieval might become highly dependent on that one identifier. If the top-scoring n-gram matches many passages or matches the wrong passage, the remaining low-scoring n-grams provide insufficient corrective evidence.
...
Across 6,515 queries, the number of n-grams matched to the top-1 retrieved passage has a mean of 41.5 (median: 42.0, $\sigma$: 6.3), with a range of 11 to 65. Table~\ref{tab:ngram-count} presents retrieval performance stratified by n-gram count intervals.

\begin{table}[htbp]
    \centering
    \caption{Retrieval success rate and mean n-gram corpus frequency ($\bar{F}_c$) by number of matched n-grams in the top-1 passage.}
    \label{tab:ngram-count}
    \begin{tabular}{lrrr}
        \toprule
        \textbf{N-gram Count Range} & \textbf{Queries} & \textbf{Success@1} & \textbf{Mean $\bar{F}_c$} \\
        \midrule
        $<$30   & 273   & 31.5\% & 387,506 \\
        31--38  & 1,668 & 38.9\% & 379,866 \\
        39--44  & 2,554 & 44.4\% & 385,187 \\
        45--52  & 1,798 & 42.3\% & 381,124 \\
        $>$52   & 222   & 38.7\% & 365,687 \\
        \bottomrule
    \end{tabular}
\end{table}

The relationship between n-gram count and retrieval success is notably weak (Spearman $\rho = 0.040$, $p = 0.001$), exhibiting a non-monotonic, inverted-U pattern. Success rates increase from 31.5\% for passages with low identifier counts ($<$30) to a peak of 44.4\% in the 39--44 range, subsequently declining to 38.7\% for passages exceeding 52 matched n-grams. This trend suggests that while a minimum amount of evidence is required to identify a passage, accumulating n-grams beyond a moderate threshold yields diminishing or negative returns.

This performance plateau points to a structural vulnerability in \glsxtrshort{SEAL}'s additive scoring function, $W(d, q)$. Because the document score is computed as a summation of individual n-gram weights without normalization for quantity, in high-count scenarios, a passage may be ranked first not due to high precision, but because it contains a large volume of generic domain vocabulary that the model associates with the query context.

Notably, the mean n-gram corpus frequency ($\bar{F}_c$) remains stable across all intervals (ranging from 365,687 to 387,506). This stability indicates that the decline (or increase) in success at higher counts is not caused by the generative component reverting to high-frequency "filler" tokens. Instead, the failure might come from the scoring mechanism failing to distinguish between concentrated, unique n-grams and the aggregate score of multiple, moderately relevant n-grams that may appear in non-relevant but topically-broad documents.

\subsection{Missing Answer Identifiers}
\label{sec:failure-4}

Missing Answer Identifiers assesses whether any generated n-gram contains the target answer string. This metric is particularly revealing because successful retrieval theoretically \textit{requires} that n-grams identify passages containing the answer, yet generation of the answer string itself is neither guaranteed nor strictly necessary, as the model generates n-grams to identify documents, not to answer questions directly.

Of 6,515 queries analyzed, the answer string appears in at least one generated n-gram for the top-1 retrieved passage for only 1,096 queries (16.8\%), while 5,419 queries (83.2\%) fail to generate any n-gram containing the answer in their respective highest-ranked retrieved passage. When the answer appears in the n-grams, the success rate reaches 75.0\%, compared to 34.9\% when the answer is absent among the n-grams.

This is revealing, as the model is trained to generate distinctive, query-relevant n-grams, but it is not explicitly trained to generate the answer itself. Consequently, even when the model successfully identifies topically relevant passages (e.g., generating ``Supreme Court,'' ``justices,'' ``current composition'' for a query about the number of justices), it may never generate the specific answer token (e.g., ``nine''). In such cases, the model retrieves passages from the correct topical domain but lacks the precise discriminative n-gram needed to confidnetly isolate the answer-bearing passage from related alternatives.

The low Missing Answer Identifiers rate (16.8\%) suggests that most of the successful retrievals rely on ``indirect'' identification, where the model generates n-grams describing the context surrounding the answer rather than the answer itself. While this approach can succeed when the context is sufficiently distinctive, it may fail when multiple documents share similar contextual terms but differ in their specifics.
